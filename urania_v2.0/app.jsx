...
function App() {
  ...
  // Save session — use computedResult as fallback because result state is reset
  // to null when navigating fresh to results (only kept for reopened sessions).
  const saveSession = useCallback(() => {
    const toSave = result || computedResult;
    if (!toSave) return;
    ...
    const session = {
      ...
      intentions, result: toSave,
    };
    const next = [...sessions, session];
    setSessions(next);
    saveSessions(next);
    setSavedId(id);
  }, [result, computedResult, flow, you, partner, intentions, sessions]);
...
noSpinStyle.textContent = `.no-ring-spin .ring-cosmic { animation: ring-glow-pulse 3s ease-in-out infinite alternate !important; }`;
